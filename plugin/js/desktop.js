...
(function($, PLUGIN_ID) {
    ...
    var MULTICHOICE_ROW_NUM;
    var CHECKBOX_ROW_NUM;
    ...
    function convertOldConfigtoNewConfig(old_conf) {
        ...
        MULTICHOICE_ROW_NUM = 10;
        CHECKBOX_ROW_NUM = 10;

        var new_conf = {};
        new_conf.checkbox_row_number = old_conf.line_number;
        new_conf.multichoice_row_number = old_conf.line_number;
        ...
        for (var i = 1; i < Number(old_conf.line_number) + 1; i++) {
            new_conf['multichoice_row' + i] = {};
            new_conf['checkbox_row' + i] = {};
            new_conf['multichoice_row' + i].field = old_conf.body_text['cfield_text_' + i]['value'];
            new_conf['multichoice_row' + i].type = old_conf.body_text['ctype_text_' + i]['value'];
            new_conf['multichoice_row' + i].value = old_conf.body_text['cvalue_text_' + i]['value'];
            new_conf['multichoice_row' + i].targetfield = old_conf.body_text['tfield_text_' + i]['value'];
            new_conf['multichoice_row' + i].targetcolor = old_conf.body_text['tcolor_text_' + i]['value'];
            new_conf['multichoice_row' + i].targetbgcolor = old_conf.body_text['tbgcolor_text_' + i]['value'];
            new_conf['multichoice_row' + i].targetsize = old_conf.body_text['tsize_text_' + i]['value'];
            new_conf['multichoice_row' + i].targetfont = '';
            new_conf['checkbox_row' + i].field = old_conf.body_date['cfield_date_' + i]['value'];
            new_conf['checkbox_row' + i].type = old_conf.body_date['ctype_date_' + i]['value'];
            new_conf['checkbox_row' + i].value = convert.abs(old_conf.body_date['cvalue_date_' + i]['value']);
            new_conf['checkbox_row' + i].type2 = convert.type2(Number(old_conf.body_date['cvalue_date_' + i]['value']));
            new_conf['checkbox_row' + i].targetfield = old_conf.body_date['tfield_date_' + i]['value'];
            new_conf['checkbox_row' + i].targetcolor = old_conf.body_date['tcolor_date_' + i]['value'];
            new_conf['checkbox_row' + i].targetbgcolor = old_conf.body_date['tbgcolor_date_' + i]['value'];
            new_conf['checkbox_row' + i].targetsize = old_conf.body_date['tsize_date_' + i]['value'];
            new_conf['checkbox_row' + i].targetfont = '';
        }
        return new_conf;
    }

    if (!CONFIG.hasOwnProperty('line_number')) {
        MULTICHOICE_ROW_NUM = Number(CONFIG['multichoice_row_number']);
        CHECKBOX_ROW_NUM = Number(CONFIG['checkbox_row_number']);
        for (var t = 1; t < MULTICHOICE_ROW_NUM + 1; t++) {
            CONFIG['multichoice_row' + t] = JSON.parse(CONFIG['multichoice_row' + t]);
        }
        for (var d = 1; d < CHECKBOX_ROW_NUM + 1; d++) {
            CONFIG['checkbox_row' + d] = JSON.parse(CONFIG['checkbox_row' + d]);
        }
    } else {
    ...
    function convertOldStatustoNewStatus(record) {
        ...
        for (var i = 1; i < 11; i++) {
            CONFIG['multichoice_row' + i].field = convert.status(CONFIG['multichoice_row' + i].field);
            CONFIG['multichoice_row' + i].targetfield = convert.status(CONFIG['multichoice_row' + i].targetfield);
            CONFIG['checkbox_row' + i].targetfield = convert.status(CONFIG['checkbox_row' + i].targetfield);
        }
    }
    ...
    function checkIndexConditionFormat(records) {
        ...
        for (var ti = 1; ti <= MULTICHOICE_ROW_NUM; ti++) {
            text_obj = CONFIG['multichoice_row' + ti];
        ...
        for (var di = 1; di <= CHECKBOX_ROW_NUM; di++) {
            date_obj = CONFIG['checkbox_row' + di];
